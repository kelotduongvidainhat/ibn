#!/bin/bash
# network/scripts/internal_config_freeze.sh
# Performs the Fetch -> Decode -> Freeze Policies -> Compute Delta -> Encode dance.
...
echo "❄️ Freezing policies for $MSP_ID..."
# We replace Admins, Writers, and Endorsement with a rule that cannot be satisfied by this Org's credentials.
# Note: We leave 'Readers' intact so the Org can still see the rejection on the ledger.
jq ".channel_group.groups.Application.groups.\"$MSP_ID\".policies.Admins.policy.value.identities[0].principal.msp_identifier = \"ForbiddenMSP\" |
    .channel_group.groups.Application.groups.\"$MSP_ID\".policies.Writers.policy.value.identities[0].principal.msp_identifier = \"ForbiddenMSP\" |
    .channel_group.groups.Application.groups.\"$MSP_ID\".policies.Endorsement.policy.value.identities[0].principal.msp_identifier = \"ForbiddenMSP\"" $CONFIG_JSON > $MODIFIED_JSON
