...
echo "--- Launching Orderer, Peer, CLI, Monitoring (Prometheus/Grafana), and ELK Stack ---"
docker compose -f "${NETWORK_DIR}/compose/docker-compose-base.yaml" -f "${NETWORK_DIR}/compose/docker-compose-org1.yaml" up -d orderer.example.com peer0.org1.example.com couchdb0 cli prometheus grafana elasticsearch logstash kibana filebeat
